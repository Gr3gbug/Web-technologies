<?php
    ...
    switch($_REQUEST['page']) {
        ...
        // Base case
        case 0:
            $body = new Template("skins/dtml/edit_group_report.html");
        ...
        // Select Case
        case 1:
            $body = new Template("skins/dtml/edit_group.html");

            $db->query("SELECT * FROM groups WHERE id = {$_GET['id']}");
            if (!$db->isError()) {
                $data = $db->getResult();
        ...
        case 3:
            ...
            $db->query("DELETE groups FROM groups WHERE id = {$_GET['id']}");
            if (!$db->isError()) {
                foreach($_POST as $key => $value) {
                    $body->setContent($key,$value);
                }
            ...
            $body->setContent("message", ($db->isError()? "KO":"OK"));
            header('Location: http://localhost/my_project_TdW/lux_backend/edit_group.php');
            exit;
        break;
